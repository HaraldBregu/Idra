...
beforeEach(() => {
	...
	mcpApi.getRoot.mockResolvedValue('/home/user/.idra/mcp/servers');
	mcpApi.registry.mockResolvedValue({
		servers: [
			...
			},
			{
				id: 'local',
				source: 'local',
				path: '/home/user/.idra/mcp/servers/local',
				data: {
					type: 'stdio',
					name: 'Local files',
					command: 'node',
...
describe('MCP settings', () => {
	it('shows remote and local servers in one simple list', async () => {
		...
		expect(screen.queryByText('/home/user/.idra/mcp/servers/local')).not.toBeInTheDocument();
