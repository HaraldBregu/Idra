...
jest.mock('../../../../src/main/agent/knowledge/wiki/wiki_location', () => ({
	wikiLocation: () => '/tmp/idra-wiki-provenance-data',
}));
...
describe('wiki evidence provenance', () => {
	it('computes excerpt hashes and rejects locator, hash, and archive tampering durably', async () => {
		const root = await mkdtemp(path.join(os.tmpdir(), 'idra-wiki-provenance-'));
		const target = path.join(root, 'wiki');
		const archive = path.join(root, 'evidence.md');
		const content = '# Evidence\nIdra stores durable facts.\nA final line.\n';
		...
		expect(verified).toEqual({
			...
			locator: 'lines 2-2',
			evidenceType: 'direct',
			excerptHash: createHash('sha256').update('Idra stores durable facts.').digest('hex'),
		});
		await expect(
			verifyWikiEvidence(
				...
				pages: [
					{
						...
						content: 'Idra stores durable facts.',
						sources: ['evidence.md'],
						claims: [
							{
								id: 'claim-durable',
								statement: 'Idra stores durable facts.',
								evidence: [
									{
										sourceId: source.sourceId!,
										locator: 'lines 2-2',
	...
	it('rolls back stale claims and publishes lineage only after a successful transaction', async () => {
		const root = await mkdtemp(path.join(os.tmpdir(), 'idra-wiki-lineage-'));
		const target = path.join(root, 'wiki');
