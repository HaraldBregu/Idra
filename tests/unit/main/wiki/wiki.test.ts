...
describe('wiki source ingestion', () => {
	it('collects supported text files and keeps source page names stable across content changes', async () => {
		const root = await mkdtemp(path.join(os.tmpdir(), 'idra-wiki-source-'));
	...
	it('rejects source symlinks that escape the configured folder', async () => {
		const root = await mkdtemp(path.join(os.tmpdir(), 'idra-wiki-symlink-'));
	...
	it('rejects oversized sources instead of truncating them', async () => {
		const root = await mkdtemp(path.join(os.tmpdir(), 'idra-wiki-oversized-'));
	...
	it('writes generated pages, schema, index and append-only log artifacts', async () => {
		const target = await mkdtemp(path.join(os.tmpdir(), 'idra-wiki-target-'));
		const source: WikiSource = {
			absolutePath: '/tmp/raw/notes.md',
			relativePath: 'notes.md',
			content: 'Idra is a desktop assistant.',
			hash: 'abc123',
		};
			...
			pages: [
				{
					path: sourcePage,
					title: 'Idra notes',
					summary: 'Notes about Idra.',
					content: 'Idra connects to [[Desktop assistants]].',
					sources: ['notes.md'],
				},
				{
					...
					content: 'Connected from [[Idra notes]].',
					sources: ['notes.md'],
				},
			],
		});
