...
describe('store preload API', () => {
	...
	it('invokes store API methods through typed IPC channels', async () => {
		...
		mockedIpcRenderer.invoke
			...
			expect(mockedIpcRenderer.invoke).toHaveBeenNthCalledWith(4, StoreChannels.saveAssistantOperator, providers[0], model);
			expect(mockedIpcRenderer.invoke).toHaveBeenNthCalledWith(5, StoreChannels.saveAgentService, providers[0], model);
			expect(mockedIpcRenderer.invoke).toHaveBeenNthCalledWith(6, StoreChannels.saveSpeechTranscriberService, providers[0], model);
	});
