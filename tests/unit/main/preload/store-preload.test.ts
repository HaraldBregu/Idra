...
describe('store preload API', () => {
	...
	it('invokes store API methods through typed IPC channels', async () => {
		...
		await expect(store.saveTextToSoundService(providers[0], model)).resolves.toBe(true);
		...
		expect(mockedIpcRenderer.invoke).toHaveBeenNthCalledWith(4, StoreChannels.saveAgentService, providers[0], model);
		expect(mockedIpcRenderer.invoke).toHaveBeenNthCalledWith(5, StoreChannels.saveSpeechTranscriberService, providers[0], model);
		expect(mockedIpcRenderer.invoke).toHaveBeenNthCalledWith(6, StoreChannels.saveTextToSoundService, providers[0], model);
	});
