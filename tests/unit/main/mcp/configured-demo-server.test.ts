...
describe('configured demo MCP server', () => {
	...
	it('loads the required server values from its manifest', () => {
		expect(demo).toMatchObject({
			id: 'idra-configured-demo',
			data: {
				type: 'stdio',
				env: {
					DEMO_COMPANY: 'Idra Studio',
					DEMO_CURRENCY: 'EUR',
					DEMO_TAX_RATE: '22',
					DEMO_SIGN_OFF: 'The Idra Studio team',
				},
			},
		});
	});
	...
	it('uses server values together with tool inputs', async () => {
		...
		expect(configuration.content).toEqual([
			expect.objectContaining({ text: expect.stringContaining('Company: Idra Studio') }),
		]);
		...
		expect(message.content).toEqual([
			expect.objectContaining({ text: expect.stringContaining('The Idra Studio team') }),
		]);
	});
});
