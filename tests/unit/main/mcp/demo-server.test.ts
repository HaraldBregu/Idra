...
describe('demo MCP server', () => {
	...
	beforeAll(async () => {
		client = await connect('idra-demo', demo, 5_000);
	});
	...
	it('runs every demo tool', async () => {
		const echo = await callTool(client, 'echo', { message: 'Hello Idra' });
		...
		expect(echo.content).toEqual([{ type: 'text', text: 'Hello Idra' }]);
