...
it('keeps the previous generation active when publication fails', () => {
	...
	try {
		...
		expect(() =>
			store.publish({
				...publication,
				generation: 'idra-failed',
				records: [publication.records[0], publication.records[0]],
			})
		).toThrow();

		expect(store.getIndex('knowledge-base')?.generation).toBe('idra-first');
...
it('exports and purges only the exact active namespace or local index', () => {
	...
	try {
		...
		expect(store.exportIndex('knowledge-base', 'idra-first')).toEqual(publication);
		expect(store.purge('knowledge-base', 'another-generation')).toEqual({
			records: 0,
			indexRemoved: false,
		});
		expect(store.getIndex('knowledge-base')).toBeDefined();
		expect(store.purge('knowledge-base', 'idra-first')).toEqual({
			records: 2,
			indexRemoved: true,
		});
