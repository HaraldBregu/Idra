...
describe('resolveSkillResource', () => {
	it('resolves contained files and rejects traversal, absolute paths, and escaping symlinks', () => {
		...
		try {
			...
			expect(resolveSkillResource(root, 'references/guide.md')).toBe(fs.realpathSync(path.join(root, 'references', 'guide.md')));
			expect(() => resolveSkillResource(root, '../secret.txt')).toThrow('outside');
			expect(() => resolveSkillResource(root, path.join(outside, 'secret.txt'))).toThrow('relative path');
