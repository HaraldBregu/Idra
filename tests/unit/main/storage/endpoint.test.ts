...
describe('normalizeEndpoint', () => {
	it('removes the configured bucket from the end of an S3 endpoint', () => {
		expect(
			normalizeEndpoint('https://account.r2.cloudflarestorage.com/friday-storage', 'friday-storage')
	...
	it('preserves endpoints that do not include the configured bucket', () => {
		expect(normalizeEndpoint('https://account.r2.cloudflarestorage.com', 'friday-storage')).toBe(
