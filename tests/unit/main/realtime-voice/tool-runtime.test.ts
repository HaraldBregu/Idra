...
it('runs native Realtime function calls through the existing tool runner and emits normalized lifecycle events', async () => {
	const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'idra-voice-tool-success-'));
...
it('persists failed Realtime tool calls with their canonical input and error outcome', async () => {
	const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'idra-voice-tool-error-'));
	const location = path.join(temporaryRoot, 'agent');
...
it('preserves the existing permission request identity and returns rejected tool status', async () => {
	const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'idra-voice-tool-denied-'));
	const location = path.join(temporaryRoot, 'agent');
	...
	try {
		runtime.handle({
			...
			name: 'write_file',
			arguments: '{"path":"/etc/idra-test","content":"test"}',
		});
		...
		expect(call).toMatchObject({
			id: 'call-permission',
			name: 'write_file',
			args: { path: '/etc/idra-test', content: 'test' },
			result: { isError: true },
		});
