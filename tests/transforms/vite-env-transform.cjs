 ...
 *   import.meta.glob(…)   →  {}   (returns empty record; tests mock the module instead)
 */
'use strict';
...
const tsJestTransformer = createTransformer({
	tsconfig: require('path').resolve(__dirname, '../../tsconfig.node.json'),
	useESM: false,
});

/**
 * Rewrite Vite-specific import.meta.* syntax to expressions that are safe in
 * a plain Node / CommonJS context.
 */
...
module.exports = {
	...
	getCacheKey(sourceText, sourcePath, options) {
		const rewritten = rewriteViteSyntax(sourceText);
		return tsJestTransformer.getCacheKey(rewritten, sourcePath, options);
	},
};
