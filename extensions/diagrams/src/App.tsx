...
export default function App() {
	...
	const [bindFunctions, setBindFunctions] =
		useState<Awaited<ReturnType<typeof renderDiagram>>['bindFunctions']>();
	const generation = useRef(0);
	const effectiveTheme =
		state.theme === 'auto' ? (fridayTheme.isDark ? 'dark' : 'default') : state.theme;
	...
	useEffect(() => {
		...
		getDiagramTypes()
			.then((types) => active && setDiagramTypes(types))
			.catch(() => undefined);
		return () => {
			active = false;
		};
	...
	const exportPng = () =>
		void svgToPng(svg)
			.then((png) => downloadFile('diagram.png', png, 'image/png'))
	...
	const fit = () => {
		...
		setZoom(
			Math.max(
				...
				Math.min(
					1,
					(container.clientWidth - 64) / (bounds.width / zoom),
					(container.clientHeight - 64) / (bounds.height / zoom)
				)
			)
		);
	};
		...
		<main className="diagrams">
			<header className="app-bar">
				...
				<label>
					...
					<select
						...
						onChange={(event) => {
							...
							if (example) update('source', example.source);
							event.target.value = '';
						}}
					>
				...
				<div className="view-switch" role="group" aria-label="Workspace view">
					{(['source', 'split', 'preview'] as ViewMode[]).map((view) => (
						<button
							key={view}
				...
				<button className="primary" disabled={rendering} onClick={() => void renderNow()}>
					Render <kbd>⌘↵</kbd>
				</button>
			</header>
			<div className="options-bar">
				<label>
					...
					<select
						value={state.theme}
						onChange={(event) => update('theme', event.target.value as AppTheme)}
					>
				...
				<label>
					...
					<select
						value={state.look}
						onChange={(event) => update('look', event.target.value as DiagramLook)}
					>
						{lookChoices.map((look) => (
				...
				<label>
					...
					<select
						value={state.layout}
						onChange={(event) => update('layout', event.target.value as DiagramLayout)}
					>
						{layoutChoices.map((layout) => (
							<option key={layout}>{layout}</option>
						))}
					</select>
				</label>
				<span className="renderer-count" title={diagramTypes.join(', ')}>
					{diagramTypes.length || '…'} registered renderers
				</span>
				<span className="spacer" />
				<button
					onClick={() => downloadFile('diagram.mmd', state.source, 'text/plain;charset=utf-8')}
				>
					Save source <kbd>⌘S</kbd>
				</button>
			</div>
			<div className={workspaceClass}>
				<Editor
					...
					onConfigChange={(value) => update('configText', value)}
					onImport={importFile}
				/>
				<Preview
					svg={svg}
					...
					onZoomChange={(value) => setZoom(Math.max(0.2, Math.min(4, value)))}
					onFit={fit}
					onCopy={copySvg}
					onSvg={exportSvg}
					onPng={exportPng}
			...
			<footer className="status-bar" aria-live="polite">
				<span className={error ? 'status-error' : 'status-ok'}>
					{error ? 'Render failed' : rendering ? 'Rendering…' : svg ? 'Diagram ready' : 'Ready'}
				</span>
				{diagramType && <span>{diagramType}</span>}
				{!hasAccessibilityText && (
					<span
						className="status-warning"
						title="Add accTitle and accDescr to the Mermaid source for an accessible exported SVG."
					>
