...
if (tokenPath) {
	...
	console.log('projects:', await projects.list({ onState: (state) => console.log('state:', state) }));
	process.exit(0);
}
...
const server = createServer(async (req, res) => {
	...
	if (channel === 'agent:send') {
		...
		const emit = (event) =>
			stream.write(`data: ${JSON.stringify({ channel: 'agent:response', data: { runId, ...event } })}\n\n`);
