...
try {
	...
	await page.screenshot({ path: path.join(SHOT_DIR, '70-empty-rect.png') });
	console.log('screenshot: 70-empty-rect');

	await page.evaluate(() => document.querySelector('.tiptap').focus());
	await page.keyboard.type('Some text to see the send button state', { delay: 5 });
	await page.waitForTimeout(300);
	await page.screenshot({ path: path.join(SHOT_DIR, '71-typed-rect.png') });
	console.log('screenshot: 71-typed-rect');
} finally {
