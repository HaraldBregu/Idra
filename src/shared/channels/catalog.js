import { CHANNEL_PROVIDER_IDS } from './definitions';
export const CHANNEL_RUNTIME_SUPPORT_VALUES = ['bundled', 'catalog-only'];
export const CHANNEL_CATALOG_EXPOSURES = ['stable', 'preview', 'hidden'];
...
const SETUP_FIELDS_BY_CHANNEL_ID = {
    clickclack: [
        'serverUrl',
        'token',
        'secret',
...
const CHANNEL_CATALOG_INPUT = [
    ...
    entry('googlechat', 'Google Chat', 'Connect Google Chat spaces and DMs.', ['gchat', 'google-chat'], 40, true, 'preview', 'google_chat'),
    ...
    entry('msteams', 'Microsoft Teams', 'Connect Microsoft Teams chats and channels.', ['teams'], 100, true, 'preview', 'microsoft_teams'),
    entry('nextcloud-talk', 'Nextcloud Talk', 'Connect Nextcloud Talk conversations.', ['nc-talk', 'nc'], 110, true),
    entry('nostr', 'Nostr', 'Connect Nostr direct messages and relays.', [], 120, false),
    entry('qa-channel', 'QA Channel', 'Synthetic channel for local QA and contract tests.', [], 130, true, 'hidden'),
...
export const CHANNEL_CATALOG = CHANNEL_CATALOG_INPUT.map((item) => ({
    ...item,
...
export const CHANNEL_BUNDLED_RUNTIME_IDS = listChannelIdsWhere((entry) => entry.runtime === 'bundled');
export const CHANNEL_CATALOG_ONLY_RUNTIME_IDS = listChannelIdsWhere((entry) => entry.runtime === 'catalog-only');
...
const CHANNEL_IDS = new Set(CHANNEL_PROVIDER_IDS);
const CHANNEL_ALIAS_TO_ID = new Map();
...
export function listChannelCatalog() {
    return CHANNEL_CATALOG;
}
export function getChannelCatalogEntry(idOrAlias) {
    const id = normalizeChannelId(idOrAlias);
    return id ? CHANNEL_CATALOG_BY_ID[id] : undefined;
}
export function getChannelBrandIconId(idOrAlias) {
    return getChannelCatalogEntry(idOrAlias)?.brandIconId;
}
export function buildChannelDocsUrl(docsPath, repositoryHomepage) {
    const normalizedPath = normalizeChannelDocsPath(docsPath);
    const normalizedHomepage = repositoryHomepage.trim().replace(/\/+$/, '');
    if (!normalizedPath || !/^https?:\/\//.test(normalizedHomepage))
        return null;
    const encodedPath = normalizedPath.split('/').map(encodeURIComponent).join('/');
    return `${normalizedHomepage}/blob/main/${encodedPath}`;
}
export function normalizeChannelId(idOrAlias) {
    const normalized = normalizeChannelKey(idOrAlias);
    return CHANNEL_ALIAS_TO_ID.get(normalized) ?? (CHANNEL_IDS.has(normalized) ? normalized : null);
}
export function isChannelId(value) {
    return normalizeChannelId(value) === value;
}
export function extractChannelCatalogFromPackageMetadata(metadata) {
    if (!metadata || typeof metadata !== 'object')
        return CHANNEL_CATALOG;
    const packageMetadata = metadata;
    const fridayChannel = packageMetadata['friday.channel'];
    if (!fridayChannel || typeof fridayChannel !== 'object')
        return CHANNEL_CATALOG;
    const catalog = fridayChannel.catalog;
...
function entry(id, label, blurb, aliases, order, markdownCapable, exposure = 'preview', brandIconId) {
    return {
        id,
        label,
        blurb,
...
function normalizeChannelDocsPath(value) {
    const normalized = value.trim().replace(/^\/+/, '');
    const segments = normalized.split('/');
    if (!normalized.startsWith('docs/channels/') ||
...
function isPackageCatalogEntry(value) {
...
function isChannelRuntimeSupport(value) {
    return value === 'bundled' || value === 'catalog-only';
}
function isChannelSetupField(value) {
    return typeof value === 'string' && CHANNEL_SETUP_FIELDS.includes(value);
}
function indexCatalogById(resolve) {
    return Object.freeze(Object.fromEntries(CHANNEL_CATALOG.map((entry) => [entry.id, resolve(entry)])));
}
function listChannelIdsWhere(predicate) {
