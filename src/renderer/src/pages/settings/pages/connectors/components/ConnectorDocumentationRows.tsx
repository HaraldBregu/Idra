...
export function ConnectorDocumentationRows({
	...
	const environmentSecretNames = connector.environmentSecretNames ?? [];
	const documentationPages = connector.platformDocumentationPages ?? [];

	return (
		<div className={cn('grid gap-3 px-3 py-2.5', className)}>
			{(connector.docsLabel || connector.docsPath) && (
			...
			{environmentSecretNames.length > 0 && (
				<div className="grid gap-1.5">
					...
					<div className="flex flex-wrap gap-1.5">
						{environmentSecretNames.map((secret) => (
