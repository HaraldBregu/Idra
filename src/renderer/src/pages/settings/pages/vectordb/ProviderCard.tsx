...
export function ProviderCard({ provider, baseUrl }: ProviderCardProps): React.JSX.Element {
	...
	const save = async (): Promise<void> => {
		...
		try {
			const saved = await window.provider.set(
				{ id: provider.id, name: provider.name, apiKey: draft.trim(), baseUrl },
				'databases'
			);
					...
					<div className="flex min-w-0 flex-1 items-center gap-2.5">
						...
						<div className="min-w-0 flex-1">
								...
								{provider.apiKeyUrl && (
									<Button
										...
										onClick={(event) => {
											...
											openExternalUrl(provider.apiKeyUrl ?? '');
										}}
									>
							...
							<p className="truncate text-xs font-medium leading-tight text-muted-foreground">
								{canonical ? MASKED_API_KEY_LABEL : t('settings.vectorDb.notConfigured')}
							</p>
						</div>
					</div>
					<CardAction
				...
				<CollapsibleContent className="flex flex-col gap-3">
					<CardContent className="space-y-4">
						...
						<section className="space-y-3">
							...
							{editing ? (
								<SettingsField
									id={`database-${provider.id}-api-key`}
