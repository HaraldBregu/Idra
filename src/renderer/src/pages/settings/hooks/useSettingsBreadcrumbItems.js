...
export function useSettingsBreadcrumbItems() {
    ...
    const connectorDetailId = location.pathname.startsWith('/settings/connectors/connectordetails/')
        ...
        : null;
    const [connectorDetailName, setConnectorDetailName] = useState(null);
    useEffect(() => {
        ...
        void window.connectors.get(connectorDetailId).then(() => {
            const name = CONNECTOR_DEFAULTS.find((entry) => entry.id === connectorDetailId)?.name ??
                connectorDetailId;
            if (mounted)
                setConnectorDetailName(name ?? null);
        }, () => {
            if (mounted)
                setConnectorDetailName(null);
        });
    ...
    if (location.pathname.startsWith('/settings/model-services/')) {
        ...
        const normalizedServiceId = serviceId === 'friday' || serviceId === 'main'
            ? AGENTS.assistant
            : serviceId;
        const serviceItem = SETTINGS_MODEL_SERVICE_ITEMS.find((item) => item.id === normalizedServiceId);
    ...
    const current = SETTINGS_NAVIGATION.find((item) => (location.pathname === item.path || location.pathname.startsWith(`${item.path}/`)));
    if (!current)
        return [];
    const items = [{ label: t(current.labelKey) }];
