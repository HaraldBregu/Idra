...
import { createInitialModelServiceState, getErrorMessage, getSelectedServiceModel, MODEL_SERVICE_DEFINITIONS, SETUP_STEPS, supportedProviderIds, } from '../constants';
import { appApi } from '@/lib/compat';
export function useModelServices(state, dispatch, connectedProviderIds, navigate) {
    const { step, serviceStates, savingConfig } = state;
    useEffect(() => {
        if (step === 'presentation' || step === 'providers')
            return;
        let cancelled = false;
        async function loadServiceModels() {
            ...
            try {
                ...
                if (cancelled)
                    return;
                const selectableProviders = storedProviders.filter((provider) => supportedProviderIds.has(provider.id));
                ...
                if (selectableProviders.length > 0) {
                    for (let i = 0; i < MODEL_SERVICE_DEFINITIONS.length; i += 1) {
                        ...
                        const preferredProvider = (selection
                        ...
                        const preferredGroup = modelGroups.find((g) => g.provider.id === preferredProvider?.id) ?? modelGroups[0];
                        const providerId = preferredGroup?.provider.id ?? '';
                        const preferredModelId = selection?.model.id;
                        const modelId = preferredGroup?.models.find((m) => m.id === preferredModelId)?.id ??
    ...
    function handleServiceProviderChange(serviceId, value) {
        const providerId = value ?? '';
        const group = serviceStates[serviceId].modelGroups.find((item) => item.provider.id === providerId);
        dispatch({ type: 'CLEAR_ERROR' });
        dispatch({
    ...
    function handleServiceModelChange(serviceId, value) {
        dispatch({ type: 'CLEAR_ERROR' });
        dispatch({ type: 'CHANGE_SERVICE_MODEL', serviceId, modelId: value ?? '' });
    }
    async function handleSaveModelStep(service, stepIndex) {
        if (savingConfig)
            return;
