...
export const MODEL_SERVICE_DEFINITIONS: readonly ModelServiceDefinition[] = [
	{
		...
		icon: Bot,
		required: true,
		getSelection: () => window.store.getAgentService(),
		getModels: (provider) => window.app.getModels(provider),
		saveSelection: (provider, model) => window.store.saveAgentService(provider, model),
	},
	{
		...
		icon: Mic,
		required: false,
		getSelection: () => window.store.getSpeechTranscriberService(),
		getModels: (provider) => window.app.getSpeechToTextModels(provider),
		saveSelection: (provider, model) => window.store.saveSpeechTranscriberService(provider, model),
	},
	{
		...
		icon: Volume2,
		required: false,
		getSelection: () => window.store.getTextToSpeechService(),
		getModels: (provider) => window.app.getTextToSpeechModels(provider),
		saveSelection: (provider, model) => window.store.saveTextToSpeechService(provider, model),
	},
	{
		...
		icon: ImageIcon,
		required: false,
		getSelection: () => window.store.getImageCreatorService(),
		getModels: (provider) => window.app.getImageCreatorModels(provider),
		saveSelection: (provider, model) => window.store.saveImageCreatorService(provider, model),
	},
	{
		...
		icon: Video,
		required: false,
		getSelection: () => window.store.getTextToVideoService(),
		getModels: (provider) => window.app.getTextToVideoModels(provider),
		saveSelection: (provider, model) => window.store.saveTextToVideoService(provider, model),
	},
	{
		...
		icon: Music,
		required: false,
		getSelection: () => window.store.getTextToSoundService(),
		getModels: (provider) => window.app.getTextToSoundModels(provider),
		saveSelection: (provider, model) => window.store.saveTextToSoundService(provider, model),
	},
];
