...
export async function getAppMicrophoneEnabled() {
    try {
...
export function mergeDictationText(baseText, transcript) {
    if (!baseText)
        return transcript;
    if (!transcript)
        return baseText;
...
export function resampleToPcm16(input, inputRate, outputRate) {
    if (input.length === 0)
...
export function createRecordingFileName(mimeType) {
...
export async function getAppMicrophonePermission() {
