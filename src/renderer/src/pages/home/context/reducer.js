import { applyAgentResponseEventToTools, agentToolPartFromHistoryBlock, updateAgentToolPart, } from './tool-parts';
import { initialAgentChatState, welcomeMessage, } from './state';
...
function createAgentMessage(id, runId, startedAtMs) {
    return {
        id,
        role: 'agent',
        type: 'agent',
...
function updateAgentMessage(state, messageId, update) {
    return {
        ...state,
        messages: state.messages.map((message) => message.id === messageId && isAgentMessage(message) ? update(message) : message),
    };
}
function activeAgent(state) {
    return state.messages.find((message) => message.id === state.activeAgentId && isAgentMessage(message));
}
function ensureAgentForRun(state, runId) {
    ...
    const existing = state.messages.find((message) => isAgentMessage(message) && message.runId === runId);
    if (existing) {
        return {
...
function applyResponseEvent(state, event, receivedAtMs) {
    if (state.activeRunId && state.activeRunId !== event.runId)
        return state;
    const ensured = ensureAgentForRun(state, event.runId);
    if (ensured.message.runId && ensured.message.runId !== event.runId)
        return state;
    ...
    if (event.type === 'text_delta') {
        if (!event.delta)
            return ensured.state;
        return updateAgentMessage({ ...ensured.state, activeAgentId: ensured.message.id, activeRunId: event.runId }, ensured.message.id, (message) => ({
            ...message,
            runId: event.runId,
            state: 'answering',
...
function addToolResultToMessages(messages, toolUseId, content, isError, status, output) {
    if (!toolUseId)
        return [...messages];
    const resolvedStatus = status ?? (isError ? 'error' : 'ok');
    const hasError = resolvedStatus !== 'ok';
    const errorText = hasError && content
        ? content
        : hasError
    ...
    for (let index = messages.length - 1; index >= 0; index--) {
        ...
        if (!isAgentMessage(message))
            continue;
        if (!message.tools.some((tool) => tool.toolCallId === toolUseId))
            continue;
        const nextMessage = {
            ...message,
        ...
        return messages.map((current, currentIndex) => currentIndex === index ? nextMessage : current);
    }
    return [...messages];
}
export function historyToChatMessages(history) {
    ...
    history.forEach((message, index) => {
        if (message.role === 'tool') {
            const next = addToolResultToMessages(out, message.toolUseId, message.content, message.isError, message.status, message.output);
        ...
        if (message.role === 'user') {
            ...
            if (content.length > 0)
                out.push(createUserMessage(`user-history-${index}`, content));
            return;
        }
        ...
        if (content.length === 0 && tools.length === 0)
            return;
        out.push({
...
export function agentChatReducer(state, action) {
    switch (action.type) {
        case 'submit_user_message': {
            const userMessage = createUserMessage(action.userMessageId, action.content);
            const agentMessage = createAgentMessage(action.agentMessageId, undefined, action.submittedAtMs);
            return {
        ...
        case 'complete_active': {
            const current = activeAgent(state);
            if (!current) {
                if (action.response.trim().length === 0)
                    return state;
                const message = {
            ...
            const nextState = updateAgentMessage(state, current.id, (message) => ({
                ...
                state: message.state === 'error' || message.state === 'cancelled'
                    ? message.state
                    : 'completed',
