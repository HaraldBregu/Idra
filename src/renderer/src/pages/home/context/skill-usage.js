...
function parseSkillLocation(value) {
    if (typeof value !== 'string')
        return undefined;
...
function addUsage(items, seen, usage) {
    if (!usage)
        return;
    const key = usage.version ? `${usage.id}@${usage.version}` : usage.id;
    if (seen.has(key))
        return;
    if (!usage.version && items.some((item) => item.id === usage.id))
        return;
    if (usage.version) {
...
function collectFromResultPayload(items, seen, payload) {
    if (!isRecord(payload))
        return;
...
export function getAgentSkillUsages(tools) {
    const items = [];
    const seen = new Set();
    for (const tool of tools) {
        ...
        if (tool.type === EXECUTE_SKILL_TOOL_NAME) {
            if (isRecord(tool.input)) {
                ...
                addUsage(items, seen, skillId && version
