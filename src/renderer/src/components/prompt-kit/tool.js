...
function stateIcon(state) {
    switch (state) {
        case 'input-streaming':
            return _jsx(Loader2, { className: "size-3.5 animate-spin text-muted-foreground" });
        case 'input-available':
            return _jsx(Settings, { className: "size-3.5 text-muted-foreground" });
        case 'output-available':
            return _jsx(CheckCircle, { className: "size-3.5 text-muted-foreground" });
        case 'output-error':
            return _jsx(XCircle, { className: "size-3.5 text-muted-foreground" });
    }
}
