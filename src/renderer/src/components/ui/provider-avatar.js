...
export function ProviderAvatar({ providerId, name, className, }) {
    const icon = providerIconAssets[providerId];
    if (icon) {
        return (_jsxs(Avatar, { className: cn('size-8 rounded-md border border-border bg-background p-1', className), children: [_jsx(AvatarImage, { src: icon.light, alt: "", "aria-hidden": "true", draggable: false, className: "size-full object-contain dark:hidden" }), _jsx(AvatarImage, { src: icon.dark, alt: "", "aria-hidden": "true", draggable: false, className: "hidden size-full object-contain dark:block" }), _jsx(AvatarFallback, { className: "rounded-md bg-muted text-sm font-semibold text-muted-foreground", children: getProviderInitial(name || providerId) })] }));
    }
    const initial = getProviderInitial(name || providerId);
    return (_jsx(Avatar, { className: cn('size-8 rounded-md bg-muted text-sm font-semibold text-muted-foreground', className), children: _jsx(AvatarFallback, { className: "rounded-md bg-muted text-sm font-semibold text-muted-foreground", children: initial }) }));
}
