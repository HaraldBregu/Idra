...
function deriveCharsPerFrame(speed) {
    return Math.max(1, Math.round(speed / 10));
}
function deriveFadeDuration(speed) {
    return Math.max(200, 2000 - clampSpeed(speed) * 16);
}
function deriveSegmentDelay(speed) {
    return Math.max(20, 240 - clampSpeed(speed) * 2);
}
function splitSegments(text) {
    return text.split(/(\s+)/).filter((s) => s.length > 0);
}
function useTextStream({ textStream, mode = 'typewriter', speed = 20, fadeDuration, segmentDelay, characterChunkSize, onComplete, onError, }) {
        ...
        const tick = (now) => {
            ...
            if (mode === 'typewriter') {
                if (indexRef.current < bufferRef.current.length) {
                    indexRef.current = Math.min(bufferRef.current.length, indexRef.current + chunkSize);
...
function ResponseStream({ textStream, mode = 'typewriter', speed = 20, className, onComplete, as = 'div', fadeDuration, segmentDelay, characterChunkSize, }) {
    ...
    if (mode === 'fade') {
        ...
        return (_jsx(Tag, { className: cn('whitespace-pre-wrap', className), children: segments.map((seg, i) => (_jsx("span", { style: {
    ...
    return _jsx(Tag, { className: cn('whitespace-pre-wrap', className), children: displayedText });
}
