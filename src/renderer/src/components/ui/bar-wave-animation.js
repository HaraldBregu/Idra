...
function getAmp(phase) {
    const t = (phase * 0.22) % CYCLE;
    let env = 0;
    for (const [c, hw] of WORDS) {
        const d = Math.abs(t - c);
        if (d < hw)
            env = Math.max(env, Math.cos((d / hw) * Math.PI * 0.5));
    }
    if (env < 0.05)
        return 0.005 + 0.004 * Math.abs(Math.sin(phase * 7.9));
...
function readAnalyserAmp(analyser, dataArray) {
...
function appendAmp(target, amp) {
    target.copyWithin(0, 1);
    target[target.length - 1] = amp;
}
function decayAmps(target) {
    for (let i = 0; i < target.length; i++) {
        target[i] = target[i] * 0.82 + 0.006 * 0.18;
    }
}
export function BarWaveAnimation({ active = true, height = 80, className, mediaStream, }) {
    ...
    useEffect(() => {
        ...
        const hairLine = new THREE.Line(hairGeo, new THREE.LineBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.13 }));
        scene.add(hairLine);
        ...
        const barMesh = new THREE.LineSegments(barGeo, new THREE.LineBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.92 }));
        scene.add(barMesh);
        let phase = 0;
        let liveAmp = 0.006;
        function redraw() {
