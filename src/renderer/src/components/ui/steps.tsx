		...
		>
			{leftIcon ? (
				<span className="inline-grid size-4 place-items-center">
					...
					{swapIconOnHover && (
						<ChevronDown className="hidden size-3 -rotate-90 text-muted-foreground transition-transform group-data-[panel-open]:rotate-0 group-hover:block" />
					)}
				</span>
			) : (
				<ChevronDown className="size-3 -rotate-90 text-muted-foreground transition-transform group-data-[panel-open]:rotate-0" />
			)}
			<span>{children}</span>
		</CollapsibleTrigger>
	);
