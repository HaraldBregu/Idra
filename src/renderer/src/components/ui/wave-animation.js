...
function updatePositions(geo, cfg, t) {
    ...
    const br = Math.max(0.18, 0.63 + 0.24 * Math.sin(t * 0.68 + cfg.ph * 0.2) + 0.13 * Math.sin(t * 1.43 + cfg.ph * 0.5));
    for (let i = 0; i <= SEGS; i++) {
        const u = i / SEGS;
        const x = u * 2 - 1;
...
export function WaveAnimation({ active = true, height = 64, className }) {
    ...
    useEffect(() => {
        ...
        WAVES.forEach((cfg, i) => {
            ...
            bakeColors(geo, cfg);
            const glowMat = new THREE.LineBasicMaterial({ vertexColors: true, transparent: true, opacity: 0.25, depthWrite: false });
        ...
        ro.observe(canvas);
        // initial size if already laid out
