...
export function ProviderAvatar({ providerId, name, className, }) {
    ...
    if (icon) {
        return (_jsxs("div", { className: cn('flex size-8 shrink-0 items-center justify-center overflow-hidden rounded-md border border-border bg-background p-1', className), children: [_jsx("img", { src: icon.light, alt: "", "aria-hidden": "true", draggable: false, className: "size-full object-contain dark:hidden" }), _jsx("img", { src: icon.dark, alt: "", "aria-hidden": "true", draggable: false, className: "hidden size-full object-contain dark:block" })] }));
    }
    const initial = getProviderInitial(name || providerId);
    return (_jsx("div", { className: cn('flex size-8 shrink-0 items-center justify-center rounded-md bg-muted text-sm font-semibold text-muted-foreground', className), children: initial }));
}
