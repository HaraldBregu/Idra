...
function parseCronExpression(expression: string): [number[], number[], number[], number[], number[]] {
	const fields = expression.trim().split(/\s+/);
	if (fields.length !== 5) {
		throw new Error('Cron expression must use 5 fields: minute hour dayOfMonth month dayOfWeek.');
	}
	return fields.map((field, index) => {
		const [min, max] = CRON_FIELD_RANGES[index]!;
		const values = parseCronPart(field, min, max);
		if (!values) throw new Error(`Invalid cron field ${index + 1}: "${field}".`);
		return values;
