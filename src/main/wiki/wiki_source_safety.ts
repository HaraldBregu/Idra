...
const ASSIGNED_SECRET = /(?:api[_-]?key|access[_-]?token|client[_-]?secret|password)\s*[:=]\s*["']?[A-Za-z0-9_\-/.+=]{20,}/i;

export function assertWikiSourceSafe(source: WikiSource): void {
	const normalized = source.relativePath.split(path.sep).join('/');
	if (SECRET_FILE.test(normalized)) throw new Error(`Refusing to ingest credential-like file: ${normalized}`);
