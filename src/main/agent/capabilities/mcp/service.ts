...
export class AgentMcpClientService implements AgentMcpClientServicePort {
	...
	constructor(
		_logger?: { warn(source: string, message: string, data?: unknown): void },
		_connectors?: unknown
	) {}
	...
	async reconnect(): Promise<ConnectorTestResult> {
		return { status: 'configured' };
	}
	...
	async callTool(_id: unknown, name: unknown): Promise<unknown> {
		throw new Error(`MCP tool is unavailable: ${String(name)}`);
	}
