...
export function createSoundTool(agentDir: string): Tool {
	return tool({
		name: 'create_sound',
		description:
			'Generate music or a sound effect from a text prompt using the configured text-to-audio provider. Saves the audio under the agent library directory and returns its path relative to the agent directory. If you reference it in markdown, use the returned path.',
