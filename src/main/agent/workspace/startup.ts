...
export class AgentStartupFilesService implements AgentStartupFilesServicePort {
	...
	async ensureReady(agentId: string): Promise<void> {
		...
		} else if (!bootstrapExists && !profileConfigured && !hadStartupFiles) {
...
async function assertSafeWritableStartupFile(
	...
	const resolvedTarget = path.resolve(parent, path.basename(filePath));
	if (!resolvedTarget.startsWith(`${resolvedRoot}${path.sep}`)) {
		throw new Error(`Refusing to write outside startup root: ${name}`);
	}
}
