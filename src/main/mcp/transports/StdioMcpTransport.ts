...
import { McpConnectionError, McpToolExecutionError, normalizeMcpError } from '../errors';
import { createSafeMcpEnv } from '../env';
import {
...
export class StdioMcpTransport implements IMcpTransportAdapter {
		...
		await withTimeout(
			...
			async (signal) => {
				...
				const transport = new StdioClientTransport({
					...
					env: createSafeMcpEnv(this.config.env),
					cwd: this.config.cwd,
					stderr: 'pipe',
				});
				client.onerror = (error) => {
					throw new McpConnectionError(error.message, { cause: error });
				};
	...
	async callTool(name: string, args: unknown, options: McpCallOptions = {}): Promise<unknown> {
		const client = this.requireClient();
		return withRetry(options.retries ?? 0, async () =>
			withTimeout(
				`Tool ${name}`,
				options.timeoutMs ?? DEFAULT_MCP_TOOL_TIMEOUT_MS,
				async (signal) => {
					try {
						return await client.callTool(
							{ name, arguments: args && typeof args === 'object' ? (args as Record<string, unknown>) : {} },
							undefined,
							{ signal, timeout: options.timeoutMs ?? DEFAULT_MCP_TOOL_TIMEOUT_MS }
						);
					} catch (error) {
						throw new McpToolExecutionError(`Failed to execute MCP tool ${name}.`, {
							cause: normalizeMcpError(error, `Failed to execute MCP tool ${name}.`),
						});
					}
				},
				options.signal
