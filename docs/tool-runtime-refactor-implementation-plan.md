...
2. Route `runAgent` selected tools through the adapter before provider requests.
   - Verify: agent run tests prove provider schemas/names are normalized and original tools execute.
...
4. Keep full canonical migration as a separate later step.
   - Verify: no file moves and no store/session migrations.

## Follow-Up Work

- Wire the default `AgentService` tool factory into canonical `createAgentTools`.
